      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            {selected.length === 0 ? (
              <tr><td colSpan={11} className="px-3 py-10 text-center text-slate-400">선택된 품목이 없습니다.</td></tr>
            ) : (
              ESTIMATE_CATEGORIES.flatMap((category) => {
                ...
                return categoryItems.map((item) => {
                  ...
                  const profit = quotedAmount - executionTotal
                  return (
                    <tr key={item.id} className="hover:bg-slate-50">
                      ...
                      <td className="px-2 py-2 text-center text-slate-600">{formatPercent(item.margin_rate)}</td>
                      <td className="px-3 py-2 text-right font-semibold text-slate-800 whitespace-nowrap">{formatKRW(quotedAmount)}</td>
